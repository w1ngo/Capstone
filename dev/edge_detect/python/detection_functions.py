...
def find_measurements( img_filename, params ):
    ...
    # read image & filter for prep
        # as preprocessing is always the same, only do this once
        # repeating this in the loop adds a TON of exec time
    ...
    for line in params: 
        ...
        # if did not receive clear information
        if len( dat ) == 0:
            continue

        # TODO: implement box combination? or leave as a skip
        if len( dat ) > 1:
            continue
